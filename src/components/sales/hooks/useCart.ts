...
export function useCart(
  ...
  const handleApplyVoucher = async () => {
    ...
    // 1. Cek static voucher dulu (SAVE10, HEMAT20, dll)
    const staticResult = validateVoucher(upper);
    if (staticResult.valid) {
      setVoucher(staticResult);
      setVoucherError('');
      return;
    }
    ...
    try {
      ...
      const response = await apiFetch(`/api/vouchers/validate?code=${upper}`);
      const result = await response.json();

      if (result.valid) {
        setVoucher({
          valid: true,
          type: result.type === 'PERCENTAGE' ? 'percent' : 'flat',
          value: result.value,
          label: result.label,
          id: result.id,       // simpan id untuk update current_usage saat checkout
        });
        setVoucherError('');
      } else {
        setVoucher(null);
